...
for ii = 1:length(old_eventTimes)
    ...
    if summary == 1 && ii == length(old_eventTimes)
        ...
        for jj = 1:length(eventTimes)
            Bz_data = importdata(sprintf('./clean_data/IMF_Bz_%s.txt',eventTimes{jj}(1:11)),' ',1);
    ...
    for ii = 1:length(old_eventTimes)
        ...
        if summary == 1 && ii == length(old_eventTimes)
            jump = 0;
            for jj = 1:length(eventTimes)
                Bt_data = importdata(sprintf('./clean_data/IMF_Bt_%s.txt',eventTimes{jj}(1:11)),' ',1);
                Timestamp = [strcat(Bt_data.textdata(2:end,1),{' '},Bt_data.textdata(2:end,2))];
                %             if length(Bt_data.data(:,1)) ~= 20161
                %                 jump = jump + 1;
                %                 continue
                %             end
    ...
    if summary == 0
        ...
        set(gcf,'visible','off')
        saveFigure(gcf, './summary_events/all_flux08_SWBtot.png');
        ...
        set(gcf,'visible','off')
        saveFigure(gcf, './summary_events/all_flux08_SWV.png');
        ...
        set(gcf,'visible','off')
        saveFigure(gcf, './summary_events/all_flux08_Pdyn.png');
        ...
        set(gcf,'visible','off')
        saveFigure(gcf, './summary_events/all_flux20_SWV.png');
        ...
        set(gcf,'visible','off')
        saveFigure(gcf, './summary_events/all_flux20_Pdyn.png');
        ...
        set(gcf,'visible','off')
        saveFigure(gcf, './summary_events/all_flux20_ObsB.png');
...
if summary == 1
    ...
    %         pos7(3) = pos1(3);pos6(3) = pos1(3);pos5(3) = pos1(3);pos4(3) = pos1(3);pos3(3) = pos1(3);pos2(3) = pos1(3);
    %         set(ah7,'Position',pos7);set(ah6,'Position',pos6);set(ah5,'Position',pos5);set(ah4,'Position',pos4);set(ah3,'Position',pos3);set(ah2,'Position',pos2)
    %
    %     set(ah1,'XTickLabel','');set(ah2,'XTickLabel','');set(ah3,'XTickLabel','');set(ah4,'XTickLabel','');set(ah5,'XTickLabel','');set(ah6,'XTickLabel','');
